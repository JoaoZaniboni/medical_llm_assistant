...
class RagEvaluatorPack(BaseLlamaPack):
    ...
    def _create_async_evaluate_example_prediction_tasks(
        ...
        answer_relevancy_task = judges["answer_relevancy"].aevaluate(
            query=example.query,
            response=prediction.response,
        ...
        context_relevancy_task = judges["context_relevancy"].aevaluate(
            query=example.query,
            response=prediction.response,
            contexts=prediction.contexts,
            sleep_time_in_seconds=sleep_time_in_seconds,
    ...
    def _evaluate_example_prediction(self, judges, example, prediction):
        ...
        context_relevancy_result = judges["context_relevancy"].aevaluate(
            query=example.query,
            response=prediction.response,
            contexts=prediction.contexts,
        )
    ...
    def _save_evaluations(self):
        ...
        # saving evaluations
        evaluations_objects = {
            "answer_relevancy": [e.dict() for e in self.evals["context_relevancy"]],
            "correctness": [e.dict() for e in self.evals["correctness"]],
            "faithfulness": [e.dict() for e in self.evals["faithfulness"]],
            "answer_relevancy": [e.dict() for e in self.evals["answer_relevancy"]],
        }
    ...
    def _prepare_and_save_benchmark_results(self):
        ...
        _, mean_answer_relevancy_df = get_eval_results_df(
            ["base_rag"] * len(self.evals["answer_relevancy"]),
            self.evals["answer_relevancy"],
        ...
        _, mean_context_relevancy_df = get_eval_results_df(
            ["base_rag"] * len(self.evals["context_relevancy"]),
            self.evals["context_relevancy"],
    ...
    def _make_evaluations(
        ...
        for batch in self._batch_examples_and_preds(
            ...
            for example, prediction in tqdm.tqdm(zip(examples, predictions)):
                ...
                self.evals["answer_relevancy"].append(answer_relevancy_result)
                self.evals["faithfulness"].append(faithfulness_result)
                self.evals["context_relevancy"].append(context_relevancy_result)
    ...
    async def _amake_evaluations(self, batch_size, sleep_time_in_seconds):
        ...
        for batch in batch_iterator:
            ...
            self.evals["answer_relevancy"] += eval_results[1::4]
            self.evals["faithfulness"] += eval_results[2::4]
            self.evals["context_relevancy"] += eval_results[3::4]
            # update queue
